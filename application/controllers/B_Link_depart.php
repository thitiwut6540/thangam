...
class B_Link_depart extends CI_Controller {
    ...
    public function __construct(){
        ...
        $this->load->model('B_Link_depart_m');
    ...
    public function index(){
        $data['pA'] = "m_link_depart";
        $this->load->view('backoffice/template/a_header');
        $this->load->view('backoffice/template/_nav');
        $this->load->view('backoffice/link_depart.php',$data);
        $this->load->view('backoffice/template/a_footer');
    }

    public function linkdepart_list(){
        $data['Re'] = $this->B_Link_depart_m->getList();
        $this->load->view('backoffice/link_depart_fetch', $data);
    }

    public function linkdepart_insert(){
        $data['pA'] = "m_link_depart";
        $this->load->view('backoffice/template/a_header');
        $this->load->view('backoffice/template/_nav');
        $this->load->view('backoffice/link_depart_insert.php',$data);
        $this->load->view('backoffice/template/a_footer');
    }

    public function linkdepart_insert_save(){ 

        if($this->input->post('action')=="l-depart-insert"){

            $this->db->select_max('l_no', 'l_no_max');
            $Re_m = $this->db->get('tb_link_depart');
            ...
            if(!empty($_FILES['l_photo']['name'])) {
                $config['upload_path'] ='./public/images/link_depart';
                $config['allowed_types'] = 'jpg|jpeg|png';
                $config['max_size'] = 0;
                $config['file_name'] = "LD".date('YmdHis').rand(1000,9999);
                $this->upload->initialize($config);
                if($this->upload->do_upload('l_photo')){
                    $data=$this->upload->data();
                    $config['image_library'] = 'gd2';
                    $config['source_image'] = './public/images/link_depart/'.$data['file_name'];
            ...
            $this->db->insert('tb_link_depart', $data);
    ...
    public function linkdepart_edit(){
        $id=$this->uri->segment(4);
        $data['pA'] = "m_link_depart";
        $data['Re'] = $this->B_Link_depart_m->getEdit($id);

        $this->load->view('backoffice/template/a_header');
        $this->load->view('backoffice/template/_nav');
        $this->load->view('backoffice/link_depart_edit.php',$data);
        $this->load->view('backoffice/template/a_footer');
    }

    public function linkdepart_edit_save(){
        ...
        if(!empty($_FILES['l_photo']['name'])) {
            ...
            $ReCHK = $this->db->get('tb_link_depart');
        ...
        $this->db->trans_begin();
            ...
            $this->db->where('l_id', $l_id);
            $this->db->update('tb_link_depart', $data);
        ...
        if ($this->db->trans_status() === FALSE){
            ...
            $this->db->trans_commit();
            if(!empty($pDele1) AND file_exists('public/images/link_depart/'.$pDele1)){
                unlink('public/images/link_depart/'.$pDele1);
            }
    ...
    public function linkdepart_delete(){
 
        $l_id=$this->input->post('l_id');
        $sql="SELECT * FROM tb_link_depart WHERE l_id='$l_id'";
        ...
        $this->db->trans_begin();
            ...
            $this->db->delete('tb_link_depart');
        $this->db->trans_complete();
        if ($this->db->trans_status() === FALSE){
            $this->db->trans_rollback();
            $Response = array('action' => 'N','output' => 'ไม่สามารถลบรายการปุ่มลิงค์หน่วยงานได้');
            echo json_encode($Response);
            exit;
            ...
            $Response = array('action' => 'Y','output' => 'ลบรายการปุ่มลิงค์หน่วยงานเรียบร้อย');
            echo json_encode($Response);
        }
    ...
    public function linkdepart_status(){
        $l_id=$this->input->POST('l_id');
        $l_status=$this->input->POST('l_status');
