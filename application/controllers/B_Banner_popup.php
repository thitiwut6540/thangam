...
class B_Banner_popup extends CI_Controller {
    ...
    public function __construct(){
        ...
        $this->load->model('B_Banner_popup_m');
    ...
    public function index(){
        ...
        $this->load->view('backoffice/banner_popup.php',$data);
        $this->load->view('backoffice/template/a_footer');
    }

    public function banner_list(){
        $data['Re'] = $this->B_Banner_popup_m->getList();
        $this->load->view('backoffice/banner_popup_fetch', $data);
    }
    ...
    public function banner_insert(){
        ...
        $this->load->view('backoffice/template/a_header');
        $this->load->view('backoffice/template/_nav');
        $this->load->view('backoffice/banner_popup_insert.php',$data);
        $this->load->view('backoffice/template/a_footer');
    }
    ...
    public function banner_insert_save(){ 
        ...
        $Re_m = $this->db->get('tb_banner_popup');
        ...
        if(!empty($_FILES['ban_photo']['name'])) {
            $config['upload_path'] ='./public/images/banner_popup';
            $config['allowed_types'] = 'jpg|jpeg|png';
            $config['max_size'] = 0;
            $config['file_name'] = "BP".date('YmdHis').rand(1000,9999);
            $this->upload->initialize($config);
            if($this->upload->do_upload('ban_photo')){
                $data=$this->upload->data();
                $config['image_library'] = 'gd2';
                $config['source_image'] = './public/images/banner_popup/'.$data['file_name'];
        ...
        $this->db->insert('tb_banner_popup', $data);
    ...
    public function banner_edit(){
        ...
        $data['pA'] = "m_banner_popup";
        $data['Re'] = $this->B_Banner_popup_m->getEdit($id);

        $this->load->view('backoffice/template/a_header');
        $this->load->view('backoffice/template/_nav');
        $this->load->view('backoffice/banner_popup_edit.php',$data);
        $this->load->view('backoffice/template/a_footer');
    }
    ...
    public function banner_edit_save(){
        ...
        if(!empty($_FILES['ban_photo']['name'])) {
            ...
            $ReCHK = $this->db->get('tb_banner_popup');
        ...
        $this->db->trans_begin();
            ...
            $this->db->where('ban_id', $ban_id);
            $this->db->update('tb_banner_popup', $data);
        ...
        if ($this->db->trans_status() === FALSE){
            ...
            $this->db->trans_commit();
            if(!empty($pDele1) AND file_exists('public/images/banner_popup/'.$pDele1)){
                unlink('public/images/banner_popup/'.$pDele1);
            }
    ...
    public function banner_delete(){
        ...
        $ban_id=$this->input->post('ban_id');
        $sql="SELECT * FROM tb_banner_popup WHERE ban_id='$ban_id'";
        ...
        $this->db->trans_begin();
            ...
            $this->db->delete('tb_banner_popup');
        $this->db->trans_complete();
        if ($this->db->trans_status() === FALSE){
            $this->db->trans_rollback();
            $Response = array('action' => 'N','output' => 'ไม่สามารถลบรายการภาพแสดงหน้าแรก (POPUP) ได้');
            echo json_encode($Response);
            exit;
            ...
            $Response = array('action' => 'Y','output' => 'ลบรายการภาพแสดงหน้าแรก (POPUP) เรียบร้อย');
            echo json_encode($Response);
        }
