...
class Corrupt extends CI_Controller {
    ...
    public function corrupt_save(){ 
        ...
        $Re=$this->M_Corrupt_m->getCorruptInsert($this->input->POST(),$photoName1,$photoName2);
    ...
    public function corrupt_list(){
        $this->load->view('template/a_header');
        $this->load->view('template/a_nav');
        $this->load->view('template/a_menu_main');
        $this->load->view('corruption_list');
        $this->load->view('template/a_menu_link');
        $this->load->view('template/a_footer');
    }

    public function corrupt_list_fetch(){
        $search = '';
        $this->load->library('pagination');
		$limit = 50;
        $offset = ($this->uri->segment(3)) ? $this->uri->segment(3) : 0;

        $config['base_url'] = base_url('M_Corrupt/corrupt_list_fetch');
        $config['total_rows'] = $this->M_Corrupt_m->getCorruptList($limit, $offset, $search, $count=true);
		...
		$data['Re'] = $this->M_Corrupt_m->getCorruptList($limit, $offset, $search, $count=false);
		$data['pagelinks'] = $this->pagination->create_links();
        $this->load->view('corruption_list_fetch', $data);
    }


    public function corrupt_detail(){
        $data['Re'] = $this->M_Corrupt_m->getCorrupt($this->uri->segment(3));

        $this->load->view('template/a_header');
        $this->load->view('template/a_nav');
        $this->load->view('template/a_menu_main');
        $this->load->view('corruption_detail',$data);
        $this->load->view('template/a_menu_link');
        $this->load->view('template/a_footer');
    }
