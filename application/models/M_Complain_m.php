<?php
class M_Complain_m extends CI_Model {
    ...
    function getComplainInsert($data,$photoName1,$photoName2){
        ...
        if(!empty($data['c_type']) AND !empty($data['c_title']) AND !empty($data['c_cus_name']) AND !empty($data['c_cus_id']) AND !empty($data['c_cus_tel']) ){
            $this->db->trans_begin();
                ...
                $sql = "SELECT MAX(c_no) AS max_no FROM tb_complain WHERE c_cata='R' AND c_no like '$yid%'";
            ...
            if ($this->db->trans_status() === FALSE){
                ...
                $Response = array('action' => 'N','output' => 'ไม่สามารถส่งเรื่องร้องเรียนร้องทุกข์ได้<br>กรุุณาติดต่อเจ้าหน้าที่');
                return $Response;
                exit;
            }else{
                ...
                $Response = array('action' => 'Y','output' => 'แจ้งเรื่องร้องเรียนร้องทุกข์เรียบร้อย');
                return $Response;
                exit;
            }
        }else{
    ...
    function getComplainList($limit, $offset, $search, $count){

        $this->db->select("c_id");
        $this->db->from("tb_complain");
        $this->db->where("c_cata", "R");
        $query = $this->db->get();
