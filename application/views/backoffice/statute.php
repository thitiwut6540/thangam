<div class="container-fluid">
    <div id="panel" class="toggled">
        <div id="sidebar">
            <?php $this->load->view('backoffice/statute_menu');?>
        </div>
    </div> 
            ...
            <div class="col-12">
                <div class="box_con">
                    <div class="box_con_header">
                        <div class="row">
                            <div class="col-8"><i class="fas fa-book"></i> <?php echo $type; ?></div>
                            <div class="col-4 text-right"><a class="btn btn-sm btn-success" href="<?php echo base_url('backoffice/กฎหมายและระเบียบ/'.$type.'/insert');?>"><i class="fas fa-plus"></i> เพิ่ม<?php echo $type;?></a></div>
                        </div>
                    </div>
...
<script>
    $(document).ready(function() {
        ...
        function ajaxList(page_url = false){
            var baseurl = '<?php echo base_url("B_Statute/statute_list"); ?>';
            ...
            $.confirm({
                ...
                buttons: {
                    ลบ: {
                        ...
                        action: function() {
                            $.ajax({
                                url: '<?php echo base_url("B_Statute/statute_delete"); ?>',
                                method: "POST",
                                dataType: "json",
                                data: { stt_id: stt_id},
                                success: function(data) {
            ...
            $.confirm({
                ...
                buttons: {
                    เปลี่ยนสถานะ: {
                        ...
                        action: function() {
                            $.ajax({
                                url: '<?php echo base_url("B_Statute/statute_approve"); ?>',
                                method: "POST",
                                dataType: "json",
                                data: { stt_id: stt_id, status:status},
                                success: function(data) {
                                    if(data.action=='Y'){
                                        $.alert({
                                            icon: 'fas fa-check',
