                    ...
                    <div class="box_con_header">
                        <div class="row">
                            ...
                            <div class="col-4 text-right">
                                <a class="btn btn_green" id="btn_insert" href="<?php echo base_url('Backoffice/กฎหมายและระเบียบ/'.$topic.'/เพิ่ม')?>"><i class="fas fa-plus"></i> เพิ่ม</a>
...
<script>
    $(document).ready(function() {
        ajaxLawslist(page_url = false);

        $(document).on('click', ".pagination li a", function(event) {
            var page_url = $(this).attr('href');
            ajaxLawslist(page_url);
            event.preventDefault();
        });

        function ajaxLawslist(page_url = false){
            var page = $('#topic').val();
            var baseurl = base_url+"B_Laws/list";
